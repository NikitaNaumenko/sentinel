defmodule SentinelWeb.SessionLive.LoginTest do
  ...
  describe "Log in page" do
    test "renders log in page", %{conn: conn} do
      ...
      assert html =~ dgettext("session", "Log in")
      assert html =~ dgettext("session", "Sign up")
      assert html =~ dgettext("session", "Forgot your password?")
    end
  ...
  describe "login navigation" do
    test "redirects to registration page when the Register button is clicked", %{conn: conn} do
      ...
      {:ok, _login_live, login_html} =
        ...
        |> render_click()
        |> follow_redirect(conn, ~p"/registration")
