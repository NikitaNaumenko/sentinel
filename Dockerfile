...
RUN curl -fsSL https://deb.nodesource.com/setup_21.x | bash - && apt-get update -y && apt-get install -y build-essential git nodejs && \
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/sentinel ./
